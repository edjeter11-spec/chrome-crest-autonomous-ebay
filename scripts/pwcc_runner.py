...
Phase-1 diagnosis upgrades: full stealth profile, homepage warm-up, block
detection with screenshot + body dump, telemetry.
"""
...
PWCC_HOME = "https://www.pwccmarketplace.com/"
...
def scrape_pwcc_query(page, query: str):
    ...
    blocked, reason = detect_block(page, "PWCC")
    if blocked:
        return [], reason
    ...
    items = page.evaluate("""
        () => {
            ...
            const cards = document.querySelectorAll(
                'article, [data-testid*="result"], [class*="ListingCard"], [class*="SearchResult"], [class*="card"], [class*="Item"], li'
            );
            cards.forEach(el => {
...
def main():
    ...
    with sync_playwright() as p:
        ...
        page = ctx.new_page()
        apply_stealth(page)
        warm_up(page, PWCC_HOME, "PWCC")

        for query in QUERIES:
            try:
                items, reason = scrape_pwcc_query(page, query)
            except Exception as e:
    ...
    log.info(f"PWCC DONE: upserted {added} rows (seen {seen_count})")

    write_telemetry(
        conn, "PWCC", started_at,
